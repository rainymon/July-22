...
ACTIVE_TIMELINE_SECONDS = 46.0
LEAD_IN_SECONDS = 1.0

SPECIAL_ARTICLES = {
    1:   {"file": "assets/article_001.jpg", "duration": 3.5, "scroll": True},
    6:   {"file": "assets/article_006.jpg", "duration": 1.2, "scroll": False},
    100: {"file": "assets/article_100.jpg", "duration": 1.6, "scroll": True},
    161: {"file": "assets/article_161.jpg", "duration": 1.2, "scroll": False},
    162: {"file": "assets/article_162.jpg", "duration": 2.0, "scroll": True},
    176: {"file": "assets/article_176.jpg", "duration": 2.8, "scroll": True},
}
...
.controls { width:min(900px,96vw); display:flex; align-items:center; gap:8px; margin-bottom:10px; font-size:12px; color:#565656; }
...
#stage { position:relative; width:min(900px,96vw); aspect-ratio:4/5; overflow:hidden; background:#fff; border:1px solid #d8d8d3; box-shadow:0 16px 44px rgba(0,0,0,.10); outline:none; }
...
.date,.counter { font-size:clamp(15px,2vw,28px); font-weight:650; letter-spacing:-.02em; }
.counter { text-align:right; }
.live-time { font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace; font-size:clamp(18px,2.35vw,34px); font-weight:600; letter-spacing:-.03em; }
.timeline { position:absolute; left:5.2%; right:5.2%; top:13.2%; height:5.4%; z-index:4; }
...
.timeline-labels { display:flex; justify-content:space-between; margin-top:1.2%; color:#81817c; font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace; font-size:clamp(8px,.85vw,13px); }
.feed-wrap { position:absolute; left:5.2%; right:5.2%; top:19.2%; bottom:8.5%; overflow:hidden; }
.feed { position:absolute; left:0; right:0; bottom:0; margin:0; padding:0; list-style:none; display:flex; flex-direction:column; gap:clamp(10px,1.3vh,18px); }
.feed-item { display:grid; grid-template-columns:7.2% 9.8% 1fr; column-gap:2%; align-items:baseline; padding-bottom:clamp(8px,.9vh,13px); border-bottom:1px solid #ecece8; opacity:.25; transform:translateY(13px); transition:opacity 260ms ease, transform 260ms ease; }
.feed-item.visible { opacity:.53; transform:translateY(0); }
.feed-item.current { opacity:1; }
.feed-num,.feed-time { color:#555550; font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace; font-size:clamp(10px,1.05vw,16px); line-height:1.4; }
.feed-title { color:#111; font-size:clamp(15px,1.8vw,27px); font-weight:430; line-height:1.35; letter-spacing:-.025em; word-break:keep-all; overflow-wrap:anywhere; }
.feed-item.current .feed-title { font-weight:650; }
...
.feed-fade-bottom { bottom:0; background:linear-gradient(rgba(255,255,255,0), #fff 84%); }
...
.media-viewport { position:relative; flex:1; overflow:hidden; border:1px solid #d9d9d4; background:#fafaf8; }
.media-viewport img { position:absolute; left:0; top:0; width:100%; height:auto; display:block; transform:translateY(0); will-change:transform; user-select:none; -webkit-user-drag:none; }
...
.lead { position:absolute; inset:0; z-index:40; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:1.4%; background:#fff; opacity:1; transition:opacity 380ms ease; pointer-events:auto; }
.lead.hide { opacity:0; pointer-events:none; }
.lead div { font-size:clamp(31px,5vw,72px); font-weight:650; letter-spacing:-.055em; }
.lead small { color:#666661; font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace; font-size:clamp(9px,1vw,15px); letter-spacing:.06em; }
.lead button { margin-top:2.4%; appearance:none; border:1px solid #111; border-radius:999px; background:#fff; color:#111; padding:10px 18px; font:inherit; font-size:clamp(11px,1.1vw,16px); cursor:pointer; }
#stage:fullscreen { width:min(100vw,80vh); height:auto; max-width:none; aspect-ratio:4/5; border:none; box-shadow:none; }
@media (max-width:700px) { #j22-app { padding:6px 3px 20px; } .controls { width:98vw; flex-wrap:wrap; font-size:10px; } .controls span { width:100%; } #stage { width:98vw; } }
</style>
...
(() => {
  ...
  const MAX_MINUTE = articles[articles.length - 1].minute;
  ...
  const events = articles.map(article => {
    const rank = minuteCounts.get(article.minute) || 0;
    minuteCounts.set(article.minute, rank + 1);
    const linearTime = (article.minute / MAX_MINUTE) * ACTIVE_SECONDS;
    return { ...article, eventTime: linearTime + Math.min(rank * 0.045, 0.36) };
  });

  let rafId = null, previousTimestamp = null, wallElapsed = 0, activeElapsed = 0, nextIndex = 0;
  let userPaused = false, started = false, specialRemaining = 0, currentSpecial = null, scrollMax = 0, finished = false, destroyed = false;
  let renderedItems = [];

  function pad3(v) { return String(v).padStart(3, "0"); }
  function minuteToClock(x) {
    const minute = Math.max(0, Math.min(MAX_MINUTE, Math.round(x)));
    return `${String(Math.floor(minute / 60)).padStart(2, "0")}:${String(minute % 60).padStart(2, "0")}`;
  }
  function updateClock() {
    const ratio = Math.max(0, Math.min(1, activeElapsed / ACTIVE_SECONDS));
    liveTime.textContent = minuteToClock(ratio * MAX_MINUTE);
  ...
  function addFeedItem(article) {
    ...
    while (renderedItems.length > 11) renderedItems.shift().remove();
  ...
  mediaImage.addEventListener("load", () => { recomputeScrollMax(); mediaImage.style.transform = "translateY(0px)"; });
  window.addEventListener("resize", recomputeScrollMax);
  function showSpecial(article, config) {
    currentSpecial = { duration: config.duration, scroll: !!config.scroll };
    ...
    mediaImage.style.transform = "translateY(0px)";
  ...
  function updateSpecialAnimation() {
    if (!currentSpecial || !currentSpecial.scroll) return;
    const ratio = Math.max(0, Math.min(1, (currentSpecial.duration - specialRemaining) / currentSpecial.duration));
    mediaImage.style.transform = `translateY(${-scrollMax * ratio}px)`;
  }
  function processEvents() {
    while (nextIndex < events.length && events[nextIndex].eventTime <= activeElapsed) {
      const article = events[nextIndex];
      addFeedItem(article);
      if (specialMedia[article.num]) {
        showSpecial(article, specialMedia[article.num]);
        if (article.num === 176) finished = true;
        nextIndex += 1;
        return;
      }
      nextIndex += 1;
  ...
  function reset() {
    ...
    specialRemaining = 0; currentSpecial = null; scrollMax = 0; finished = false; previousTimestamp = null;
    feed.innerHTML = ""; renderedItems = []; counter.textContent = "000"; liveTime.textContent = "00:00";
    progress.style.width = "0%"; dot.style.left = "0%"; hideSpecial(); lead.classList.remove("hide");
    pauseButton.textContent = "일시정지 · Space";
  }
  function startPlayback() { if (!started) { started = true; wallElapsed = LEAD_IN; lead.classList.add("hide"); stage.focus(); } }
    ...
    if (!userPaused && started) {
      ...
      if (wallElapsed >= LEAD_IN) {
        if (specialRemaining > 0) {
          ...
          if (specialRemaining <= 0) hideSpecial();
        } else if (!finished && activeElapsed < ACTIVE_SECONDS) {
          activeElapsed = Math.min(ACTIVE_SECONDS, activeElapsed + dt);
          updateClock();
          processEvents();
        }
      }
