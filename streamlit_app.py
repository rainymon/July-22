...
.media-viewport img {
  ...
  transform:none !important; user-select:none; -webkit-user-drag:none;
}
...
.lead { position:absolute; inset:0; z-index:40; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:1.4%; background:#fff; opacity:1; transition:opacity 380ms ease; pointer-events:none; }
  ...
  function processEvents() {
    while (nextIndex < events.length && events[nextIndex].eventTime <= activeElapsed + 1e-9) {
      ...
      if (specialMedia[article.num]) {
        ...
        if (article.num === 176) {
          // 마지막 기사는 목록 제목을 추가하지 않고 캡처만 보여준다.
          counter.textContent = pad3(article.num);
    ...
    if (!userPaused && started) {
      ...
      if (wallElapsed >= LEAD_IN) {
        if (specialRemaining > 0) {
          ...
          if (specialRemaining <= 0) {
            specialRemaining = 0;
            if (!finished) hideSpecial();
            else startButton.textContent = "재생 완료";
          }
          ...
          while (remaining > 0 && !finished && specialRemaining <= 0) {
            ...
            if (activeElapsed + remaining >= nextEventTime - 1e-9) {
              ...
              // A screenshot has started; discard sub-frame remainder.
              if (specialRemaining > 0 || finished) break;

              // Guard against a zero-duration loop after processing same-time events.
              if (used === 0 && nextIndex >= events.length) {
                remaining = 0;
              }
            } else {
