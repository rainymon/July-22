...
intro_media = ""
intro_path = BASE_DIR / "assets/main.jpg"
if intro_path.exists():
    intro_media = file_to_data_url("assets/main.jpg")
...
<div id="j22-app">
  ...
  <main id="stage" tabindex="0">
    <section id="intro-card" class="intro-card show" aria-label="첫 방송 메인 페이지">
      <img id="intro-image" alt="열여덟의 순간 첫 방송 메인 이미지" />
    </section>
...
(() => {
  ...
  const introMedia = payload.introMedia || "";
  if (introMedia) introImage.src = introMedia;
  ...
  function beginMainTimeline() {
    ...
    if (specialMedia[firstArticle.num]) {
  ...
  function reset() {
    ...
    renderedItems = []; introReady = false;
    feed.innerHTML = ""; counter.textContent = "000"; liveTime.textContent = "00:00";
    progress.style.width = "0%"; dot.style.left = "0%"; hideSpecial();
    endCard.classList.remove("show");
    startButton.textContent = "재생 시작";
  ...
  function frame(timestamp) {
    ...
    if (!userPaused && started) {
      ...
      if (wallElapsed >= LEAD_IN) {
        if (!introReady) beginMainTimeline();
        if (specialRemaining > 0) {
          ...
          while (remaining > 0 && !finished) {
            const nextEventTime = nextIndex < events.length
              ...
              : ACTIVE_SECONDS;
            if (activeElapsed < nextEventTime) {
              const step = Math.min(remaining, nextEventTime - activeElapsed);
              activeElapsed += step;
              remaining -= step;
              updateClock();
            }
            processEvents();
            if (specialRemaining > 0 || post176Blank) break;
            if (nextEventTime >= ACTIVE_SECONDS && remaining > 0) {
