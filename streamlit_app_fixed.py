# streamlit_app.py
# 2019.07.22 — 176개 기사 제목 타임라인
#
# 본문은 제공된 기사 링크의 핵심 내용을 영상용으로 짧게 요약한 것입니다.
# 기사 전문을 그대로 복제하지 않았습니다.
#
# 동작
# - 1번부터 176번까지 발행 순서대로 기사 제목을 출력합니다.
# - 00:00~23:09의 실제 간격을 49초에 선형 압축합니다.
# - 오전의 긴 공백은 길게, 14시대 기사 집중 구간은 빠르게 보입니다.
# - 1·6·100·161·162·176번은 본문 카드가 확대됩니다.
# - 전체 길이는 약 59초입니다.
#
# 조작
# R: 처음부터 다시 시작
...
# -------------------------------------------------------------------
# 아래 다섯 문자열에 실제 기사 본문 또는 본문 발췌를 붙여넣으세요.
# 인스타 1분 미만 영상에서는 기사당 250~500자를 권장합니다.
# 줄바꿈을 넣으려면 \n 을 사용하거나 따옴표 안에서 직접 줄을 바꾸세요.
# -------------------------------------------------------------------
BODY_TEXTS = {
    1: """첫 방송을 앞두고 배우들이 각자가 생각한 관전 포인트를 전했다.

옹성우는 열여덟 청춘들이 어른이 되어 가는 순간을, 김향기는 준우의 전학으로 시작되는 새로운 관계를 주목해 달라고 말했다. 신승호는 사랑과 우정, 질투와 시기, 학업과 진로처럼 청춘 주변의 현실적인 문제와 준우·휘영 사이의 팽팽한 기 싸움을 관전 요소로 꼽았다. 강기영은 학생과 어른이 함께 성장해 가는 이야기를 강조했다.""",

    6: """배우 신승호가 서울 강남구 논현동 임피리얼팰리스호텔에서 열린 JTBC 새 월화드라마 〈열여덟의 순간〉 제작발표회에 참석해 취재진 앞에서 포즈를 취했다.""",

    100: """배우 신승호가 서울 강남구 임피리얼팰리스호텔에서 진행된 JTBC 월화드라마 〈열여덟의 순간〉 제작발표회 무대에서 취재진을 향해 인사하고 있다.""",

    161: """배우 옹성우, 김향기, 강기영, 신승호와 심나연 감독이 JTBC 월화드라마 〈열여덟의 순간〉 제작발표회에서 작품의 출발을 응원하는 파이팅 포즈를 취했다.""",

    162: """〈열여덟의 순간〉은 강제 전학 온 최준우, 홀로서기를 꿈꾸는 우등생 유수빈, 완벽함 뒤에 콤플렉스를 감춘 마휘영, 아직은 서툰 담임 오한결을 중심으로 전개된다.

준우의 전학을 계기로 새로운 인연과 대립, 첫사랑과 성장의 관계가 얽힌다. 작품은 위태롭고 미숙한 열여덟 청춘들의 감정과 그 곁의 어른들이 함께 변해 가는 과정을 담는 16부작 청춘 드라마로 소개됐다.""",

    176: """강제 전학 온 최준우는 유수빈과 처음 만나고, 반장 마휘영의 안내로 2학년 3반에 들어간다. 이후 학원 배달 중 사라진 시계의 절도범으로 몰리지만 자신을 믿어 주는 어른은 거의 없다.

준우는 휘영이 담임에게 거짓말하는 장면을 우연히 듣고 시계를 가져간 사람이 휘영이라고 추궁한다. 휘영이 적대적인 말로 맞서자 준우도 물러서지 않으면서 두 소년의 갈등이 첫 회부터 본격화됐다.""",
}

ACTIVE_TIMELINE_SECONDS = 49.0
LEAD_IN_SECONDS = 1.0
KEY_HOLD_SECONDS = 1.5
FINAL_HOLD_SECONDS = 3.0
FIRST_BODY_SECONDS = 4.5
...
for article in ARTICLES:
    article["body"] = BODY_TEXTS.get(article["num"], "")
...
payload = {
    ...
    "keyHoldSeconds": KEY_HOLD_SECONDS,
    "finalHoldSeconds": FINAL_HOLD_SECONDS,
    "firstBodySeconds": FIRST_BODY_SECONDS,
}
...
<div id="j22-app">
  ...
  <main id="stage" tabindex="0" aria-label="2019년 7월 22일 기사 제목 타임라인">
    ...
    <section id="article-card" class="article-card" aria-live="polite">
      <div class="card-rule"></div>
      <div class="card-meta">
        <span id="card-num">001</span>
        <span id="card-time">00:00</span>
      </div>
      <h1 id="card-title"></h1>
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .feed-item {
    display: grid;
    grid-template-columns: 7.2% 9.8% 1fr;
    column-gap: 2%;
    align-items: baseline;
    padding-bottom: clamp(8px, .9vh, 13px);
  ...
  .feed-fade-top {
    top: 0;
    background: linear-gradient(#fff 8%, rgba(255,255,255,0));
  }

  .feed-fade-bottom {
    bottom: 0;
    background: linear-gradient(rgba(255,255,255,0), #fff 84%);
  }
...
(() => {
  ...
  if (window.__j22Timeline && typeof window.__j22Timeline.destroy === "function") {
    window.__j22Timeline.destroy();
  }
  ...
  const KEY_HOLD = payload.keyHoldSeconds;
  const FINAL_HOLD = payload.finalHoldSeconds;
  const FIRST_BODY = payload.firstBodySeconds;
  ...
  const events = articles.map(article => {
    ...
    return {
      ...article,
      eventTime: linearTime + Math.min(rank * 0.045, 0.36),
    };
  });
  ...
  let keyPauseRemaining = 0;
  let firstCardRemaining = 0;
  ...
  function minuteToClock(minuteFloat) {
    const minute = Math.max(0, Math.min(MAX_MINUTE, Math.round(minuteFloat)));
  ...
  function updateClock() {
    ...
    const currentMinute = ratio * MAX_MINUTE;
    liveTime.textContent = minuteToClock(currentMinute);
  ...
  function addFeedItem(article) {
    ...
    num.textContent = pad3(article.num);
    ...
    li.append(num, time, title);
    feed.appendChild(li);
    renderedItems.push(li);

    while (renderedItems.length > 11) {
      renderedItems.shift().remove();
    }

    requestAnimationFrame(() => {
      li.classList.add("visible", "current");
    });

    counter.textContent = pad3(article.num);
  }

  function showCard(article, durationSeconds) {
    cardNum.textContent = pad3(article.num);
    cardTime.textContent = article.time;
    cardTitle.textContent = article.title;
    cardBody.textContent = article.body || "";
    card.classList.add("show");

    if (article.num === 1) {
      firstCardRemaining = durationSeconds;
    } else if (article.num === 176) {
      finalRemaining = durationSeconds;
    } else {
      keyPauseRemaining = durationSeconds;
    }
  }
  ...
  function processEvents() {
    while (nextIndex < events.length && events[nextIndex].eventTime <= activeElapsed) {
      ...
      if (article.num === 1) {
        showCard(article, FIRST_BODY);
      } else if (article.num === 6 || article.num === 100 || article.num === 161 || article.num === 162) {
        showCard(article, KEY_HOLD);
        nextIndex += 1;
        return;
      } else if (article.num === 176) {
        showCard(article, FINAL_HOLD);
        finished = true;
        nextIndex += 1;
        return;
      }
  ...
  function frame(timestamp) {
    ...
    if (!userPaused && started) {
      ...
      if (wallElapsed >= LEAD_IN) {
        ...
        if (firstCardRemaining > 0) {
          firstCardRemaining -= dt;
          if (firstCardRemaining <= 0) hideCard();
        }

        if (keyPauseRemaining > 0) {
          keyPauseRemaining -= dt;
          if (keyPauseRemaining <= 0) hideCard();
